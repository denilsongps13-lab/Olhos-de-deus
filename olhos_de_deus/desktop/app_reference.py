...
from .reference_cockpit import ReferenceCockpitWindow as CockpitWindow
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if QSystemTrayIcon.isSystemTrayAvailable():
        ...
        def show_status() -> None:
            ...
            tray.showMessage("Olho de Deus", f"Núcleo online · {data['operational']}/{data['total_integrations']} fases prontas · Ruflo {ruflo_state}", QSystemTrayIcon.MessageIcon.Information, 5000)

        def run_doctor() -> None:
            reports = controller.doctor(probe_services=False)
            ready = sum(bool(item["operational"]) for item in reports)
            tray.showMessage("System Doctor", f"{ready}/{len(reports)} módulos operacionais, incluindo a Fase 0", QSystemTrayIcon.MessageIcon.Information, 5000)
        ...
        tray.activated.connect(lambda reason: open_action.trigger() if reason == QSystemTrayIcon.ActivationReason.DoubleClick else None)
        tray.show()
